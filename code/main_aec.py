...
if __name__ == '__main__':
    ...
    in_dims = [d0.shape[1], d1.shape[1], d2.shape[1]]
    ...
    """autoencoder"""
    print('Predicting missing modality?:', opt.pred_missing)
    ae = FA(in_dims, num_views=3, pred_missing=opt.pred_missing).float().to(device)
    ae.fit([x0_train, x1_train, x2_train], label_train, path=savepath, num_epochs=opt.max_epoch, lr=0.001)
    y_logit, hid = ae.predict([x0_test, x1_test, x2_test], label_test, path=savepath)
