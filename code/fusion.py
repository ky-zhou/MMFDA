...
class FA(nn.Module):
    def __init__(self, in_dims, num_views, pred_missing=True):  # input2 is unavailable in testing
        ...
        if self.pred_missing:
            for i in range(3):
                if i < 2:
                    ...
                    enc = nn.Sequential(
                        nn.Linear(in_dims[i], 64),
                        nn.ReLU(),
                        nn.BatchNorm1d(64, affine=True),
                    )
                self.encoder.append(enc)
                self.decoder.append(nn.Linear(hid_dec, in_dims[i]))

        else:
            for i in range(self.num_views):
                if i < 2:
                    enc = nn.Sequential(
                            nn.Linear(in_dims[i], hid1),
                            nn.ReLU(),
                            nn.BatchNorm1d(hid1, affine=True),
                            nn.Linear(hid1, hid2),
                            nn.ReLU(),
        ...
        fuse_hid = 2*hid2 + 64 if num_views==3 else 2*hid2
        self.fusion = nn.Sequential(nn.Linear(fuse_hid, num_views*hid3), nn.ReLU())
    ...
    def forward(self, x):
        if self.pred_missing:
            x_preds, zs = [], []
            for i in range(self.num_views):
                z = self.encoder[i](x[0]) if i < 2 else self.encoder[i](x[i])
                zs.append(z)
            z = torch.cat(zs, 1)
